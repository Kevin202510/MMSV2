...
void sendDataToDB()   //CONNECTING WITH MYSQL
 {
   if (client.connect(server, 80)) {
    ...
    // Make a HTTP request:
    dataToAddDBreview = (temperaturePostName+temperatureData+humidityPostName+humidityData+lightPostName+photovalue+co2PostName+gasSensor.getPPM());
    ...
    String tempsenseVal = client.readStringUntil('=');
    // Serial.println();
    String isont = client.readStringUntil('=');
    isOnTemp = isont.toInt();
    // Serial.print(isOnTemp);
    String lightsenseVal = client.readStringUntil('=');
    // Serial.println();
    String isonl= client.readStringUntil('=');
    isOnLight = isonl.toInt();
    // Serial.print(lightsenseVal.length());

    if(temperatureSensorVal.length()==21){
    ...
    if(tempsenseVal.length()==11){
      if(isOnTemp==1){
        Serial.println("Sensor mo OFF");
        digitalWrite(relay, LOW); 
      }else{
        Serial.println("Sensor mo ON");
        digitalWrite(relay, HIGH); 
      }
    }
    if(lightsenseVal.length()==13){
      if(isOnLight==1){
        Serial.println("Sensor mo OFF");
